...
class OrthonormalMultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # Attention layers - ensure exact dimensions
        self.q_proj = nn.Linear(d_model, num_heads * self.d_k, bias=False)
        self.k_proj = nn.Linear(d_model, num_heads * self.d_k, bias=False)
        self.v_proj = nn.Linear(d_model, num_heads * self.d_k, bias=False)
    ...
    def _get_cached_positional_bias(self, seq_len: int, device: torch.device) -> torch.Tensor:
        """Efficient caching of positional bias, only active during eval mode."""

        if not self.training:  # <<<< KEY CHANGE: Only use cache if model is in eval mode
            # Clear cache if device changed OR if it's the first call on this device in eval mode
            ...
            # Compute positional bias for eval mode
            # No explicit torch.no_grad() here as _compute_positional_bias itself might need grads
            # for parameters if they were updated. The key is it's not backwarded multiple times.
            pos_bias = self._compute_positional_bias(seq_len, device)

            # Only cache reasonable sizes to avoid memory issues, respecting self.max_seq_len if set
            # Using a fixed 1024 as per your original, or self.max_seq_len from init
            cache_limit = getattr(self, 'max_seq_len', 1024)  # Fallback to 1024 if max_seq_len not an attribute
            if seq_len <= cache_limit:
                self._cached_pos_bias[cache_key] = pos_bias

            return pos_bias
        else:
            # Always recompute positional bias during training
            # This ensures a fresh computation graph for each backward pass
            # when gradient accumulation is used.
            return self._compute_positional_bias(seq_len, device)
    ...
    def forward(self,
        ...
        # Project Q, K, V with explicit dimension check
        q_out = self.q_proj(x)  # Should be (batch_size, seq_len, num_heads * d_k)
        k_out = self.k_proj(x)  # Should be (batch_size, seq_len, num_heads * d_k)
        v_out = self.v_proj(x)  # Should be (batch_size, seq_len, num_heads * d_k)

        # Verify dimensions before reshaping
        expected_dim = self.num_heads * self.d_k
        if q_out.shape[-1] != expected_dim:
            raise RuntimeError(f"Q projection output dimension {q_out.shape[-1]} != expected {expected_dim}")

        q = q_out.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
        k = k_out.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
        v = v_out.view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
